...
var map = {
	...
	getTileData: function(x, y){return tiles[this.data[x][y]];},
	getMoveCost: function(x,y){return this.getTileData(x, y).movecost;},
};

function pathfind(start, destination, map, columns, rows, allow_diagonals)
{
	...
	while (open.length > 0)
	{
		...
		for (var new_node_x = Math.max(0, current_node.x-1); new_node_x <= Math.min(columns-1, current_node.x+1); new_node_x++)
			for (var new_node_y = Math.max(0, current_node.y-1); new_node_y <= Math.min(rows-1, current_node.y+1); new_node_y++)
			{
				...
				if (!map.isBlocked(new_node_x, new_node_y) //map[new_node_x][new_node_y] == 0 //If the new node is open
					|| (destination.x == new_node_x && destination.y == new_node_y)) //or the new node is our destination
				{
					...
					if (!found_in_open)
					{
						...
						new_node.g = current_node.g + map.getMoveCost(new_node_x, new_node_y); //Math.floor(Math.sqrt(Math.pow(new_node.x-current_node.x, 2)+Math.pow(new_node.y-current_node.y, 2)));
