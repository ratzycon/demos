...
demo.setup = function(){
	...
	// todo:
	// multiple exploded mass attractors - if photon collision, reflect out photon
	// mass increase on photon hit? (to simulate real mass particles pushed along by photon energy)
	// heavy objects attract more photon collisions and sends out more photons
	// distant objects gets pushed away by photon storms?
	...
	grid = pointField(10,10,10,24);
	photons = pointField(8,8,8,64);
	...
	photons.update = function(time, delta){

		for(var i = 0; i< photons.geometry.vertices.length; i++)
		{
			var photon = photons.geometry.vertices[i];
			photon.z += 1.0;
			//photon.x += 0.5;
			//photon.y += 0.2;
			...
			if(photon.z<-size)
			{
				photon.z = size + Math.random()*8;
				photon.x = photon.x * Math.random();
				photon.y = photon.y * Math.random();
			}
			...
			if(photon.y>size)
				photon.y = -size - Math.random()*8;


		}
		photons.geometry.verticesNeedUpdate = true;
	};

	updateList.push(grid);
	updateList.push(photons);
}

var target = scene, cradius = 600, cspeed = .077;
...
function createAttractorTest(){

	uniforms.numAttractors = 1;

	var a = new THREE.Object3D();
	scene.add(a);

	a.position = new THREE.Vector3(0,0,0);
	a.mass = 7700;
	a.attractPower = 2.0;
	a.attractDirection = 1.0;
	...
	attractors.push(a);
	updateList.push(a);
	a.update = createUpdate(a, 0);

	var sphere = geosphere(13.0, 1 );
	scene.add(sphere);
	sphere.parent = a;
}
...
function createAttractors(num){
	...
	for(var i = 0; i<num;i++)
	{
		...
		/*a.model = geosphere(33, 3);
		a.model.position = a.position;
		scene.add(a.model);*/
...
function createUpdate(object, i){
	var ra = 177,// 200 * Math.random(),
	 sp = 0.4,//+(Math.random()-.5)*7,
	 offset = 0,//Math.random()*100,
	 raxis = new THREE.Vector3(Math.random(),Math.random(),Math.random());
	var rot = 0;
	return function(time, delta){ 
		//var m = Math.sin(time)*2000;
		//uniforms.attractorMass.value[i] += m;
		//uniforms.attractDirection.value[i] = Math.sin(time);
		//object.attractorMass = Math.sin(time) * 20000;
...
function geosphere(radius, segments){
	radius = radius || 1,
	segments = segments || 3;
